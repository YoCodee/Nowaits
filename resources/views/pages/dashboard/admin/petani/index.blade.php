    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="relative bg-white p-4 sm:p-6 rounded-2xl overflow-hidden mb-8 shadow-sm border border-gray-100">
            <div class="relative z-10">
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800 mb-1">Management Petani</h1>
                <p class="text-gray-500">Kelola data petani yang terdaftar di Nowaits.</p>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @foreach($petanis as $petani)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-4 font-bold text-gray-900">{{ $petani->name }}</td>
                                <td class="px-6 py-4 text-gray-600">{{ $petani->email }}</td>
                                <td class="px-6 py-4 text-gray-600">{{ $petani->no_telepon ?? '-' }}</td>
                                <td class="px-6 py-4 text-gray-600">
                                    {{ Str::limit($petani->alamatPengguna->alamat_lengkap ?? '-', 30) }}
                                </td>
